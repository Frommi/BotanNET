...
def create_group(bot, update, args):
    ...
    try:
        ...
        bot.send_message(text=answers.already_exists_name % "группа")
...
def delete_group(bot, update, args):
    if too_few_args(bot, get_chat_id(update), args, 1) is None:
        return
    if re.fullmatch("\w+", args[0]) is None:
        bot.send_message(text=answers.incorrect_name)
        return
    try:
        BD.delete_group(get_user_id(update), args[0])
    except GroupNotExistsException:
        bot.send_message(text=answers.no_exists_name % "группа")
    except UserIsNotAdmin:
        bot.send_message(text=answers.delete_no_admin % "группу")
...
def unjoin_user(bot, update, args):
    ...
    try:
        ...
        bot.send_message(text=answers.delete_no_admin % "пользователя")
